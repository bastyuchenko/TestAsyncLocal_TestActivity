...
{NewLine}").CreateLogger();
        ...
        {
            using var activity = new MyActivity("");
            activity.Start();
            //using var _ = LogContext.PushProperty("CorrelationId", MyActivity.Current?.Id);
            //using var __ = LogContext.PushProperty("TraceId", MyActivity.Current?.TraceId);
            using var ___ = LogContext.PushProperty("SpanId", MyActivity.Current?.SpanId);
